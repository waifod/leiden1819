...
\begin{proof}
    $(c)$ Fixed $b_0\in B$ as a base point, we see that $F_{b_0}=\pi_1(B,b_0)$.
    Let $e_0=const_{b_0}$.

    Remember that $\pi_{n-1}(F_{b_0},e_0)=\pi_{n-1}(\Omega B,const_{b_0})\cong
    \pi_n(B,b_0)$, hence we will define an action of $\pi_1(B,b_0)$ on
    $\pi_{n-1}(F_{b_0},e_0)$ and we will get what we want.

    Now, for an arbitrary $[\gamma]\in\pi_1(B,b_0)$, we can see it as an element
    of $F_{b_0}$. Considered an element $[\beta]\in\pi_{n-1}(F_{b_0},e_0)$, we know
    from $(a,b)$ that it naturally defines an endomorphism
    $\pi_{n-1}(F_{b_0},e_0)\xrightarrow{\alpha_{\beta}}\pi_{n-1}(F_{b_0},e_0)$.

    Consider $[\gamma]\in\pi_1(B,b_0)$.
\end{proof}
